...
6. <i class='ent'></i> [Publish your project](#Publish-project-to-annotators).


## Create a project

When you are creating a project, you can save your progress at any time. Optionally, you don't need to import your data and set up the labeling interface all at the same time.
...
3. If you are ready to import your data, click **Data Import** and import data from the Label Studio UI. For details about import formats and data types, see [Get data into Label Studio](tasks.html).
4. If you are ready to set up the labeling interface, click **Labeling Setup** and choose a template or create a custom configuration for labeling. See [Set up the labeling interface for your project](setup.html).
5. When you are done, click **Save** to save your project.
...
After you save a project, any other collaborator with access to the Label Studio instance can view your project, perform labeling, and make changes. 

!!! attention "Tip"
    To use role-based access control, you need to use Label Studio Enterprise edition.
...
<i class='ent'></i> 
Some annotation settings are only available in Label Studio Enterprise edition. If you are using Label Studio Community edition, see <a href="label_studio_compare.html">Label Studio Features</a> to learn more.
...
3. Under **Distribute Labeling Tasks**, select one of the following:
    - Auto: It is the default option to distribute tasks automatically to annotators.
...
!!! note
    You cannot assign annotators to tasks unless you select the **Manual** option. 
...
For example, if you want all tasks to be annotated by at least two annotators:
- Set the minimum number of annotations to **2**
- Enforce the overlap for 100% of tasks.

If you want at least half of the tasks to be annotated by at least three people:
- Set the minimum number of annotations to **3**
- Enforce the overlap for 50% of tasks.

If you are using the manual distribution of tasks, annotators with tasks assigned to them label those tasks first, then Label Studio automatically distributes the remaining tasks to the project annotators so that the desired overlap and a minimum number of annotations per task can be achieved.
...
3. Select your preferred method of task sampling:
    ...
    - Sequential sampling is the default, where tasks are shown to annotators in the same order that they appear on the Data Manager.
    - Uniform sampling, where tasks are shown to annotators in random order.
...
<i class='ent'></i> Review settings and the review stream are only available in Label Studio Enterprise edition. If you are using Label Studio Community edition, see <a href="label_studio_compare.html">Label Studio Features</a> to learn more.
...
In Label Studio Enterprise, you can hide projects from annotators so that you can fully configure the project before annotators can start labeling. When you are ready for annotators to start labeling, publish the project to annotators.
