...
Deploy Label Studio Enterprise on a Kubernetes Cluster using Helm 3. Use the Helm chart to set up Label Studio Enterprise for deployment onto a Kubernetes cluster and install, upgrade, and manage the application. 
Your Kubernetes cluster can be self-hosted or installed on Amazon EKS. For more information about deploying an app on Amazon EKS, see the Amazon tutorial on how to [Deploy a Kubernetes Application with Amazon Elastic Container Service for Kubernetes](https://aws.amazon.com/getting-started/hands-on/deploy-kubernetes-app-amazon-eks/).   
...
Use the following steps, if you want to install Label Studio Enterprise on Kubernetes and you have unrestricted access to the internet from your K8s cluster. 
...
Your Kubernetes cluster can be self-hosted or installed somewhere such as Amazon EKS. If you are using Amazon Elastic Kubernetes Service (EKS), see [Install Label Studio Enterprise on Amazon Elastic Kubernetes Service (EKS)](install_enterprise_aws_eks.html). 

### Capacity planning

To plan the capacity of your Kubernetes cluster, refer to the following guidelines. 
...
| Use case                              | Action                                                                                                                                       |
|---------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| More than two concurrent annotators           | Adjust the requests and limits for `resources` in the `app` pod                                                                                   |
...
You must configure `values.yaml` file for your Label Studio Enterprise deployment. The following file contains default values for a minimal installation of Label Studio Enterprise. The Helm chart has been tested and confirmed to work with the [NGINX Ingress Controller](https://kubernetes.github.io/ingress-nginx/) and [cert-manager](https://cert-manager.io/docs/).
...
!!! note
    - If `POSTGRE_SSL_MODE: verify-ca`, the server is verified by checking the certificate chain up to the root certificate stored on the client. 
    
    - If `POSTGRE_SSL_MODE: verify-full`, the server host name will be verified to make sure it matches the name stored in the server certificate. 
...
!!! note
    If you are using self-signed certificates that the host cannot verify, then you have to set `redisSslCertReqs` to `None`.
...
To restart your Helm release, do the following steps from the command line:
...
3. Run the following from the command line to upgrade your deployment:
   ...
   ```
   This command overrides the tag value stored in `lse-values.yaml`. 
   

!!! attention "important"
    To avoid version downgrades, you must update the tag value when you upgrade or redeploy your instance.
