...
> Note: For a large dataset, the HTTP request to retrieve predictions might be interrupted by a timeout. If you want to **get all predictions** for all tasks in a dataset, the recommended way is to make a [PATCH call to the tasks endpoint of the Label Studio API](/api/#operation/api_tasks_partial_update) with `"predictions": [...]` field on the ML backend side for each generated prediction.
