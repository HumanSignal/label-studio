...
Integrate your model development pipeline with your data labeling workflow by adding a machine learning (ML) backend to Label Studio. You can set up your favorite ML frameworks to do the following:
- **Pre-labeling** by allowing models to predict labels and then have annotators perform further manual refinements. 
- **Auto-labeling** by allowing models to create automatic annotations. 
- **Online Learning** by simultaneously updating your model while new annotations are created, allowing you to retrain your model on-the-fly. 
...
Use the Label Studio ML backend to integrate Label Studio with ML models. The Label Studio ML backend is an SDK that you can use to wrap your ML model code and turn it into a web server. You can then connect that server to a Label Studio instance to perform the following two tasks:
...
1. Set up an ML backend. You can either:
   - [Start an example ML backend](#Quickstart-with-an-example-ML-backend) 
      OR [Create your machine learning](ml_create.html) backend and then [Start your custom ML backend](#Start-your-custom-ML-backend-with-Label-Studio).
2. [Connect Label Studio to the ML backend](#Add-an-ML-backend-to-Label-Studio).


## Quickstart with an example ML backend

Label Studio includes several examples ML backends with popular ML models. See the [machine learning tutorials](ml_tutorials.html). Each example ML backend uses Docker Compose to start running the example ML backend server.

To start an example ML backend with Docker Compose, do the following:
1. Make sure `port 9090` is available.
2. Clone the Label Studio ML Backend Git repository. From the command line, run the following:
  ```bash
...
4. Start Docker Compose. From the command line, run the following:
    ...
    docker-compose up
    ```
The ML backend server starts and becomes available at `http://localhost:9090`. You can now [add the machine learning backend to Label Studio](#Add-an-ML-backend-to-Label-Studio) and [set up your project](setup_project.html).


## Start your custom ML backend with Label Studio

After you [create your machine learning](ml_create.html) backend](ml_create.html), you can start the ML backend server by following these instructions.
...
1. Clone the Label Studio ML Backend Git repository. From the command line, run the following:
  ```bash
   git clone https://github.com/heartexlabs/label-studio-ml-backend  
   ```
...
After you start an ML backend server, add it to your Label Studio project.
...
Using the project ID and the URL for the ML backend, you can also [add an ML backend using the API](/api/#operation/api_ml_create).
...
- Manually using the API, cURL the API from the command line. Specify the ID of the ML backend and run the following command: 
   ```
   curl -X POST http://localhost:8080/api/ml/{id}/train
   ```
  

   For more information, see [the Train API documentation](/api/#operation/api_ml_train_create).

!!! warning
    **Deprecated in version 1.4.1:**
      Automatically after any annotations are submitted or updated. Enable the option `Start model training after annotations submit or update` the **Machine Learning** settings for your project. This option will be removed in a future version of Label Studio because you can [trigger training with webhooks](ml_create.html#Trigger-training-with-webhooks).

- In development mode, training logs appear in the web browser console. 
- In production mode, you can find runtime logs in `my_backend/logs/uwsgi.log` and RQ training logs in `my_backend/logs/rq.log` on the server running the ML backend, which might be different from the Label Studio server.
...
1. Set up your ML backend for ML-assisted labeling.
   ...
   - Click **Add Model** or select **Edit** for an existing ML backend.
   - Type a **Title** for the ML backend.
...
- Using the UI: For a specific project, select the tasks that you want to delete predictions for and select **Delete predictions** from the drop-down menu.
- Using the API: Run the following command from the command line to delete the predictions for a specific project ID:
```
