...
label-studio
```
The default web browser opens automatically at [local host port 8080](http://localhost:8080) with Label Studio. For more information, see [start Label Studio](start.html) regarding options to start Label Studio.
...
If you want to build a local image, run the following command:
```bash
...
This starts Label Studio with a PostgreSQL database backend. You can also use a PostgreSQL database without Docker Compose. For more information, see [Set up database storage](storedata.html).
...
Many bugs might be fixed in patch releases or maintenance releases. 

!!! note
    Make sure to run the latest version of Label Studio by upgrading your installation before you start Label Studio. 
