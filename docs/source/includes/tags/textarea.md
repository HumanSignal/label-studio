...
| [transcription] | <code>boolean</code> | <code>false</code> | When set to true and used with `editable="true"`, the TextArea will remain an editable field even after you add your text |
| [skipDuplicates] | <code>boolean</code> | <code>false</code> | Show a pop-up warning and prevent duplicate values. See [the example below](#Example-Enforce-unique-values) |
| [displayMode] | <code>tag</code> \| <code>region-list</code> | <code>tag</code> | Display mode for the TextArea field; `region-list` shows it for every region in the Regions pane. See [the example below](#Example-Region-list-TextArea-fields) |
| [rows] | <code>number</code> | <code>1</code> | Number of rows in the TextArea field. If `1`, you can submit text by pressing Enter. If greater than `1`, you can submit text by clicking **Add** or pressing Shift + Enter |
