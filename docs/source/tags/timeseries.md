...
!!! note 
    The time axis in your data must be sorted, otherwise the `TimeSeries` tag does not work. To use auto-generated indices as time axes, don't use the `timeColumn` parameter.
...
The `Channel` tag can be used to label time series data.
...
Labeling configuration for time series data stored in a CSV loaded from a URL containing three columns: time, sensor1, and sensor2. The time column stores time as a number.
