...
!!! note
    The time axis in your data must be sorted, otherwise the `TimeSeries` tag does not work. To use autogenerated indices as time axes, don't use the `timeColumn` parameter.

Use with the following data types: time series.

### Parameters
<i> Table 1: Parameters used in `TimeSeries` tag. </i>
...
### Parameters
<i> Table 1: Parameters used in `Channel` tag. </i>
