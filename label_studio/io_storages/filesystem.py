...
def already_exists_error(what, path):
    raise RuntimeError(f'{path} {what} already exists. Use "--force" option to recreate it.')
...
class TasksJSONStorage(JSONStorage):
    ...
    def __init__(self, path, project_path, **kwargs):
        super().__init__(project_path=project_path, path=os.path.join(project_path, "tasks.json"))
...
class AnnotationsDirStorage(DirJSONsStorage):
    ...
    def __init__(self, name, path, project_path, **kwargs):
        super().__init__(name=name, project_path=project_path, path=os.path.join(project_path, "annotations"))
