...
class TestGetOrInitializeState(TestCase):
    ...
    - FSM disabled short-circuits
    - "Temporarily disabled" mode returns the inferred state without persisting
    - Existing state wins over inferred state
    - Cold start initialization executes the expected transition
    """

    def setUp(self):
        self.mock_entity = MockEntity()

    def test_get_or_initialize_state_returns_none_when_fsm_disabled(self):
        """FSM off should return None and avoid any StateManager work."""
    ...
    def test_get_or_initialize_state_returns_inferred_state_when_temporarily_disabled(self):
        """When CurrentContext disables FSM, we return inferred_state without persisting."""
        from fsm.utils import get_or_initialize_state
    ...
    def test_get_or_initialize_state_returns_none_when_inference_failed(self):
        """If inferred_state is None and there is no existing state, return None."""
        from fsm.utils import get_or_initialize_state
        ...
        with patch('fsm.utils.is_fsm_enabled', return_value=True):
            with patch('fsm.utils.CurrentContext') as mock_context:
                ...
                with patch('fsm.state_manager.get_state_manager') as mock_get_sm:
                    mock_sm = Mock()
                    mock_sm.get_current_state_value.return_value = None
                    mock_get_sm.return_value = mock_sm

                    assert get_or_initialize_state(self.mock_entity, user=None, inferred_state=None) is None
                    mock_sm.execute_transition.assert_not_called()

    def test_get_or_initialize_state_initializes_state_via_transition(self):
        """Cold start should execute a transition and return inferred_state."""
        from fsm.utils import get_or_initialize_state
            ...
            with patch('fsm.utils.CurrentContext') as mock_context:
                ...
                with patch('fsm.state_manager.get_state_manager') as mock_get_sm:
                    ...
                    with patch('fsm.utils._get_initialization_transition_name', return_value='init_transition'):
                        ...
                        assert call_kwargs.get('entity') == self.mock_entity
                        assert call_kwargs.get('transition_name') == 'init_transition'
                        assert call_kwargs.get('user') is None

    def test_get_or_initialize_state_returns_none_when_no_initialization_transition(self):
        """If no initialization transition is found, return None."""
        from fsm.utils import get_or_initialize_state
