...
class TestTaskAPI(APITestCase):
    ...
    def test_bulk_import_tasks_with_allow_skip(self):
        """Test that bulk importing tasks with allow_skip field works correctly"""
        payload = [
            {
                'project': self.project.id,
                'data': {'text': 'skippable task'},
                'meta': {},
                'allow_skip': True,
            },
            {
                'project': self.project.id,
                'data': {'text': 'unskippable task'},
                'meta': {},
                'allow_skip': False,
            },
            {
                'project': self.project.id,
                'data': {'text': 'default task'},
                'meta': {},
                # allow_skip not specified, should default to True
            },
        ]
        ...
        # Check first task (explicitly skippable)
        assert response_data[0]['allow_skip'] is True
        task1 = Task.objects.get(id=response_data[0]['id'])
        assert task1.allow_skip is True

        # Check second task (explicitly unskippable)
        assert response_data[1]['allow_skip'] is False
        task2 = Task.objects.get(id=response_data[1]['id'])
        assert task2.allow_skip is False

        # Check third task (defaults to True)
        assert response_data[2]['allow_skip'] is True
        task3 = Task.objects.get(id=response_data[2]['id'])
        assert task3.allow_skip is True
