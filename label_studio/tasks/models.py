...
from core.utils.common import find_first_one_to_one_related_field_by_prefix, load_func, string_is_url
from core.utils.params import get_env
from data_manager.managers import PreparedTaskManager, TaskManager
from django.conf import settings
from django.db import connection, models, transaction
from django.db.models import Case, Count, F, JSONField, OuterRef, Q, Subquery, Value, When
...
class Task(TaskMixin, models.Model):
    ...
    def has_lock(self, user=None):
        ...
        num_annotations = (
            self.annotations.filter(ground_truth=False).exclude(Q(was_cancelled=True) & ~Q(completed_by=user)).count()
        )
    ...
    def bulk_update_is_labeled(project):
        ...
        count = Coalesce(
            Count("annotations", filter=Q(annotations__was_cancelled=False) & Q(annotations__ground_truth=False)),
            Value(0),
        )
        condition = Case(
...
class Annotation(AnnotationMixin, models.Model):
    ...
    lead_time = models.FloatField(
        _("lead time"), null=True, default=None, help_text="How much time it took to annotate the task"
    )
    prediction = JSONField(
        _("prediction"), null=True, default=dict, help_text="Prediction viewed at the time of annotation"
    )
...
class TaskLock(models.Model):
    task = models.ForeignKey("tasks.Task", on_delete=models.CASCADE, related_name="locks", help_text="Locked task")
...
class AnnotationDraft(models.Model):
    ...
    lead_time = models.FloatField(_("lead time"), help_text="How much time it took to annotate the task")
    task = models.ForeignKey(
        "tasks.Task",
        on_delete=models.CASCADE,
        related_name="drafts",
        ...
        if isinstance(result, list):
            ...
            for tag, tag_info in project.get_control_tags_from_config().items():
                ...
                if tag_type in result:
                    return [
                        {"from_name": tag, "to_name": ",".join(tag_info["to_name"]), "type": tag_type, "value": result}
                    ]
