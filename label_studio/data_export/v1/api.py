...
from data_export.models import DataExport, Export, ConvertedFormat
    ...
    decorator=swagger_auto_schema(
        manual_parameters=[
            ...
            openapi.Parameter(
                ...
                type=openapi.TYPE_ARRAY,
                items=openapi.Schema(
                    title='Task ID', description='Individual task ID', type=openapi.TYPE_INTEGER),
                in_=openapi.IN_QUERY,
                description="""
...
class ProjectExportFiles(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        items = [{'name': p.split('/')[2].split('.')[0], 'url': p}
...
class ProjectExportFilesAuthCheck(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        generics.get_object_or_404(Project.objects.filter(
            organization=self.request.user.active_organization), pk=pk)
...
class ExportListAPI(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        task_filter_options = serializer.validated_data.pop(
            'task_filter_options')
        annotation_filter_options = serializer.validated_data.pop(
            'annotation_filter_options')
        serialization_options = serializer.validated_data.pop(
        ...
        if flag_set('fflag_fix_all_lsdv_4813_async_export_conversion_22032023_short', request.user):
            ...
            if export_type is not None and export_type != 'JSON':
                converted_file = snapshot.converted_formats.filter(
                    export_type=export_type).first()
                if converted_file is None:
                    raise NotFound(
                        f'{export_type} format is not converted yet')
...
def async_convert(converted_format_id, export_type, project, **kwargs):
    with transaction.atomic():
        try:
            converted_format = ConvertedFormat.objects.get(
                id=converted_format_id)
...
class ExportConvertAPI(generics.RetrieveAPIView):
    ...
    def post(self, request, *args, **kwargs):
        snapshot = self.get_object()
        serializer = ExportConvertSerializer(data=request.data, context={
        ...
        with transaction.atomic():
            ...
            if not created:
                raise ValidationError(
                    f'Conversion to {export_type} already started')
