...
@pytest.mark.parametrize(
    ...
    [
        [["id"], 0],  # ordered by id ascending, first element api == first created
        [["-id"], -1],  # ordered by id descending, first element api == last created
        [["completed_at"], 0],
        [["-completed_at"], -1],
        [["total_annotations"], -1],
        [["-total_annotations"], 0],
        ...
        [["annotations_results"], 0],
        [["-annotations_results"], -1],
        [["predictions_results"], 0],
        [["-predictions_results"], -1],
        [["predictions_score"], 0],
        [["-predictions_score"], -1],
        [["data.text"], 0],
        [["-data.text"], -1],
...
@pytest.mark.django_db
def test_views_ordering(ordering, element_index, business_client, project_id):
    payload = dict(
        project=project_id,
...
def test_views_filters(filters, ids, business_client, project_id):
    ...
    for _ in range(0, 2):
        task_id = make_task({"data": {"text": 1}}, project).id
