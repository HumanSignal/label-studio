...
def test_annotation_create_and_update(django_live_url, business_client):
    ...
    task_id = tasks[0]['id']
    annotation_data = {'result': [{'from_name': 'label', 'to_name': 'my_text', 'type': 'choices', 'value': {'choices': ['Positive']}}]}
    ...
    p.update_annotation(annotation_id, result=[{'from_name': 'label', 'to_name': 'my_text', 'type': 'choices', 'value': {'choices': ['Negative']}}])
