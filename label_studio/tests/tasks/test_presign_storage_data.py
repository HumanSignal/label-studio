...
class TestPresignStorageData:
    ...
    def test_missing_parameters(self, view, user):
        request = APIRequestFactory().get(reverse('data_import:storage-data-presign', kwargs={'task_id': 1}))
    ...
    def test_task_not_found(self, view, user):
        request = APIRequestFactory().get(
            reverse('data_import:storage-data-presign', kwargs={'task_id': 2}) + '?fileuri=fileuri'
        )
        request.user = user
        force_authenticate(request, user)
    ...
    def test_task_not_found(self, view, task, project, user, monkeypatch):
        ...
        request = APIRequestFactory().get(
            reverse('data_import:storage-data-presign', kwargs={'task_id': 1}) + '?fileuri=fileuri'
        )
        request.user = user
        force_authenticate(request, user)
    ...
    def test_successful_request_with_long_fileuri(self, view, task, project, user, monkeypatch):
        ...
        request = APIRequestFactory().get(
            reverse('data_import:storage-data-presign', kwargs={'task_id': 1}) + f'?fileuri={base64_encoded_uri}'
        )
