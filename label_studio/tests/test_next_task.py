...
<View>
  ...
  <Choices name="text_class" choice="single">
    ...
    ids=[
        ...
        "no annotations, first task is chosen due to active learning",
        "first task annotation, third task is chosen due to active learning",
        "first task annotation, forth task is chosen due to active learning (though task with lowest score exists but in the same cluster)",  # noqa: E501
        "lowest prediction is chosen from least solved cluster",
        "first task annotation, labeling is continued with the same cluster",
        "first task annotation, third task is chosen since cluster is marked as None (no clustering)",
        "when some of the tasks are partially labeled, regardless scores sampling operates on depth-first (try to complete all tasks asap)",  # noqa: E501
    ],
)
@pytest.mark.django_db
...
def test_breadth_first_overlap_3(business_client):
    ...
    id1 = make_task({"data": {"text": "aaa"}}, project).id  # noqa: F841
    id2 = make_task({"data": {"text": "bbb"}}, project).id  # noqa: F841
    id3 = make_task({"data": {"text": "ccc"}}, project).id  # noqa: F841
