def create_activity_annotation_template(subject,timestamp_column_name,value_column_name):
    ...
    timeseries =   f'<TimeSeries name="ts" value="$csv" valueType="url" fixedScale="True" timeColumn="{timestamp_column_name}" fixedScale="true"><Channel column="{value_column_name}"/></TimeSeries>'
    tail = '</View>'
