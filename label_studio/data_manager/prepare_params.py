...
class Column(Enum):
    ...
    UPDATED_AT = "updated_at", "Datetime", "Time the task was updated at (e.g. new annotation was created, review added, etc)"
    ANNOTATORS = (
        "annotators", "List",
    ...
    CANCELLED_ANNOTATIONS = "cancelled_annotations", "Number", "Number of cancelled or skipped annotations for the task"
    ...
    REVIEWS_REJECTED = "reviews_rejected", "Number", "Number of annotations rejected for a task in review (Enterprise only)"
    REVIEWS_ACCEPTED = "reviews_accepted", "Number", "Number of annotations accepted for a task in review (Enterprise only)"
...
class Operator(CustomEnum):
    ...
    NOT_BETWEEN = "not_in", 'Is not between min and max values, so the filter `value` should be e.g. `{"min": 1, "max": 7}`'
