...
describe("GridView", () => {
		...
		it("renders header with checkbox and ID", () => {
			...
			renderWithBEM(
				<GridHeader
					row={row}
					selected={selected}
					onSelect={selected.toggleSelected}
				/>,
			);
	...
	describe("GridDataGroup Component", () => {
		...
		it("handles array data by showing first element", () => {
			...
			expect(
				screen.getByText(JSON.stringify(row.data.array)),
			).toBeInTheDocument();
		});
	});
	...
	describe("Grid Selection Interactions", () => {
		it("handles row selection through checkbox", () => {
			...
			renderWithBEM(
				<GridHeader row={row} selected={selected} onSelect={onSelect} />,
			);
