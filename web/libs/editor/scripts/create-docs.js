...
const currentTagsUrl =
  "https://api.github.com/repos/heartexlabs/label-studio/contents/docs/source/tags";
  ...
  .then((tags) => {
    function processTemplate(t, dir, supertag) {
      ...
      if (
        !supertag &&
        t.customTags &&
        t.customTags.find((desc) => desc.tag === "subtag")
      )
        return;
      ...
      if (regions) {
        for (const region of regions.value.split(/,\s*/)) {
          const files = path.resolve(
            `${__dirname}/../src/regions/${region}.js`,
          );
          ...
          const serializeData = regionsData.find(
            (reg) => reg.name === `${region}Result`,
          );
        ...
        .replace(
          ...
          (() => {
            ...
            return (match, footnoteId) => {
              const footnoteIdx =
                footnoteIdToIdxMap[footnoteId] || ++footnoteLastIndex;
      ...
      if (supertags.includes(t.name)) {
        ...
        const templates = jsdoc2md.getTemplateDataSync({
          files: `${t.meta.path}/${t.name}/*.js`,
        });
    ...
    for (const { dir, title, nested } of groups) {
      ...
      const getTemplateDataByGlob = (glob) =>
        jsdoc2md.getTemplateDataSync({
          files: path.resolve(prefix + glob),
        });
