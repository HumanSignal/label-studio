...
Scenario(
  "Independent skip duplicate values",
  async ({ I, LabelStudio, AtSidebar, Modals }) => {
    I.amOnPage("/");
    LabelStudio.init({
      data: { letter: "Aa" },
      config: `<View>
  ...
  <TextArea name="perRegionAndAside" toName="letter" skipDuplicates="true" perRegion="true" maxsubmissions="5" displayMode="region-list"/>
    ...
    I.fillField(
      AtSidebar.locateSelectedRegion(".lsf-textarea-tag__form input"),
      "A",
    );
    I.pressKey("Enter");
...
Scenario(
  "Skip duplicate values on editing",
  async ({ I, LabelStudio, AtOutliner, Modals }) => {
    I.amOnPage("/");
  ...
  <TextArea name="perRegionAndAside" toName="letter" skipDuplicates="true" perRegion="true" maxsubmissions="5" displayMode="region-list" editable="true"/>
    ...
    I.click(
      locate('[aria-label="Edit Region"]')
        .inside('[data-testid="textarea-region"]')
        .at(2),
    ...
    I.see(
      "1",
      locate(".lsf-text-area")
        .at(1)
        .find(locate(".//*[./@data-testid = 'textarea-region'][position()=2]")),
    );
    I.dontSee(
      "A",
      locate(".lsf-text-area")
    ...
    I.see(
      ...
      locate(".lsf-text-area")
        .at(1)
        .find(locate(".//*[./@data-testid = 'textarea-region'][position()=1]")),
    );
    I.see(
      "letter",
      locate(".lsf-text-area")
    ...
    I.see(
      ...
      locate(".lsf-text-area")
        .at(1)
        .find(locate(".//*[./@data-testid = 'textarea-region'][position()=3]")),
    );

    {
      ...
      assert.deepStrictEqual(
        ...
        `There should be 3 specific text lines in the textarea result: ${JSON.stringify(
          ["A", "letter", "last"],
        )} but ${result[2].value.text} was given.`,
      );
    }

    I.say(
      "Check that skip duplication allow us to keep the same value after editing without errors",
    );
    ...
    I.say(
      "Check that skip duplication allow us to set different value by editing and do not get errors",
    );
...

    {
      ...
      assert.deepStrictEqual(
        ...
        `There should be 3 specific text lines in the textarea result: ${JSON.stringify(
          ["a", "letter", "last"],
        )} but ${result[1].value.text} was given.`,
      );
    }
    ...
    I.fillField(
      AtOutliner.locateSelectedItem(".lsf-textarea-tag__form input"),
      "One",
    );
    I.pressKey("Enter");
    I.fillField(
    ...
    I.click(
      AtOutliner.locateSelectedItem(
        locate(".lsf-textarea-tag__item:nth-child(2) input"),
      ),
    );
    I.pressKey(["CommandOrControl", "a"]);
    I.pressKey("Backspace");
    ...
    I.click(
      locate('[aria-label="Delete Region"]'),
      AtOutliner.locateSelectedItem(
        locate(".lsf-textarea-tag__item:nth-child(2)"),
      ),
    );

    I.seeInField(
      AtOutliner.locateSelectedItem(
        locate(".lsf-textarea-tag__item:nth-child(1) input"),
      ),
      "One",
    );
    I.seeInField(
    ...
    I.seeInField(
      AtOutliner.locateSelectedItem(
        locate(".lsf-textarea-tag__item:nth-child(3) input"),
      ),
      "Four",
    );
