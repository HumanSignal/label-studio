...
Scenario(
  "Make a duplicate of annotation with preselected choices",
  async ({ I, LabelStudio, AtTopbar }) => {
    const params = {
      config: `
<View>
  ...
  <Choices name="choices" toName="text">
    ...
    assert.deepStrictEqual(annotationWithPresetValues[0].value.choices, [
      "Option 1",
    ]);
  },
);
...
Scenario(
  "Make a duplicate of empty annotation with preselected choices",
  async ({ I, LabelStudio, AtTopbar }) => {
    const params = {
      config: `
<View>
