  ...
  async ({ I, LabelStudio, AtSidebar, AtImageView }) => {
    ...
    Object.keys(createShape).forEach((shapeName, shapeIdx) => {
      ...
      Object.values(createShape[shapeName]).forEach((creator) => {
        ...
        if (outerRegion.result)
          outerRegion.result[`${shapeName.toLowerCase()}labels`] = [shapeName];
        if (innerRegion.result)
          innerRegion.result[`${shapeName.toLowerCase()}labels`] = [shapeName];
      ...
      for (let i = 0; i < 2; i++) {
        if (regionPair[i].result) {
          Asserts.deepEqualWithTolerance(
            result[i].value,
            convertToImageSize(regionPair[i].result),
          );
        }
      }
    }
...
Scenario(
  ...
  async ({ I, LabelStudio, AtSidebar, AtImageView }) => {
    ...
    Object.keys(createShape).forEach((shapeName, shapeIdx) => {
      ...
      Object.values(createShape[shapeName]).forEach((creator) => {
        for (let n = 0; n < 2; n++) {
          const outerRegion = Object.values(createShape)[n].byBBox(
            50,
            50,
            size - 50 * 2,
            size - 50 * 2,
          ...
          if (outerRegion.result)
            outerRegion.result[`${outerRegion.shape.toLowerCase()}labels`] = [
              outerRegion.shape,
            ];
          if (innerRegion.result)
            innerRegion.result[`${shapeName.toLowerCase()}labels`] = [
              shapeName,
            ];
          regionPairs.push([outerRegion, innerRegion]);
        }
