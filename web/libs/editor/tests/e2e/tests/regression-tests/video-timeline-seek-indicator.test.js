...
Scenario(
  "Seek view should be in sync with indicator position",
  async ({ I, LabelStudio, AtVideoView }) => {
    I.amOnPage("/");
    LabelStudio.init({
      config: `
<View>
  ...
  <Labels name="videoLabels" toName="video">
...

    {
      ...
      const delta =
        Math.round((positionBbox.x / (trackBbox.x + trackBbox.width)) * 10) /
        10;
      ...
      assert.ok(
        indicatorBbox.x + indicatorBbox.width >=
          positionBbox.x + positionBbox.width,
        "Video position indicator not within the seek indicator when dragged near the midway point",
      );
    }
    ...
    {
      I.say(
        "Click on the seek step forward button until the video position indicator is at the end",
      );
      let indicatorPosX = indicatorBbox.x;

      I.say(
        "Move the video position indicator to the end of the seek indicator",
      );
      ...
      assert.equal(
        indicatorBbox.x,
        indicatorPosX,
        "Seeker should not have moved from this one step movement",
      );
      ...
      assert.ok(
        indicatorBbox.x > indicatorPosX,
        "Seeker should have moved from this one step movement",
      );
      indicatorPosX = indicatorBbox.x;
      ...
      assert.ok(
        indicatorBbox.x < indicatorPosX,
        "Seeker should have moved to the left from this one step movement",
      );
    }
  },
);
