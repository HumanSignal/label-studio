...
const createConfig = (tag = "Text", granularity = "symbol") => {
  return `<View>
    ...
    <${tag} name="text" value="$text" inline="true" ${
...
Scenario(
  "The selection in degenerate cases",
  async ({ I, LabelStudio, AtSidebar, AtRichText }) => {
    I.amOnPage("/");
    LabelStudio.init({
...
Scenario(
  "Non-standard characters in words",
  async ({ I, LabelStudio, AtSidebar, AtRichText }) => {
    I.amOnPage("/");
    LabelStudio.init({
      annotations: [
...
Scenario(
  "Should not select words from next line",
  async ({ I, LabelStudio, AtSidebar, AtRichText }) => {
    I.amOnPage("/");
    LabelStudio.init({
      annotations: [
    ...
    AtSidebar.seeRegions(0);
    I.pressKey("1");
    AtRichText.setSelection(
      AtRichText.locateText(),
      0,
      AtRichText.locateRoot(),
      2,
    );
    AtSidebar.seeRegions(1);
    AtSidebar.see("Оne");
...
Scenario(
  "Neighboring nested regions misplacement",
  async ({ I, LabelStudio, AtSidebar, AtRichText }) => {
...

{
  ...
  Data(startBeforeEndParams).Scenario(
    "Start before end problem",
      ...
      switch (tag.toLowerCase()) {
        case "text": {
          ...
          assert.strictEqual(
            text,
            "",
            "Region should be collapsed and the text should be empty",
          );
          break;
        }
        case "hypertext": {
