...
const scenarios = {
  "Works with data sorted ascending by time column": {
    timeseries: {
      time: [-1, 0, 1, 2, 3],
      one: [
        13.11794020069087, 95.2190411666738, 37.03620982977559,
        16.8961637786484, 49.2981075645916,
      ],
      two: [
        -5.653544080516028, 10.07458649100271, 0, -20.140046051127193,
        32.40194378594322,
      ],
    },
    assert(I) {
...
Scenario(
  "TimeSeries with optimized data",
  async ({ I, LabelStudio, AtTimeSeries }) => {
    ...
    I.say(
      "check that every timestamps from timeseries data is available to display",
    );
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (lastTimestamp !== undefined) {
        ...
        assert(
          ...
          `Timestamps should not be skipped. Got ${lastTimestamp} and ${timestamp} but ${
            timestamp - 1
          } is missed`,
        );
      }
