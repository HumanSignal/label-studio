...
async function initLabelStudio({
  ...
  window.labelStudio = new window.LabelStudio("label-studio", {
    interfaces,
    config,
    task,
    settings,
...
const clickKonva = ([x, y]) => {
  ...
  stage.fire("click", {
    clientX: x,
    clientY: y,
    evt: { offsetX: x, offsetY: y, timeStamp: Date.now() },
  });
};
