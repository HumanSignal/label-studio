...
Scenario(
  ...
  async ({ I, LabelStudio, AtOutliner, AtDetails }) => {
    ...
    I.say(
      "Cursor should be equal to `crosshair` when element is hovered and it is relation creating mode and visible",
    );
    {
    ...
    AtDetails.clickCreateRelation();
    I.say(
      "It's not relation creating mode so there should be cursor equal to `pointer`",
    );
    {
      I.say("Check the same region");
      const elementLocator = locate('[class*="htx-highlight-A"]');
...
Scenario(
  ...
  async ({ I, LabelStudio, AtOutliner, AtDetails }) => {
    ...
    I.seeElement(
      locate(".htx-highlight.__active").withChild(
        locate(".htx-highlight.__hidden").withText("r"),
      ),
    );
    ...
    I.dontSeeElement(
      locate(".htx-highlight.__hidden.__active").withChild(
        locate(".htx-highlight").withText("om"),
      ),
    );
    I.seeElement(
      locate(".htx-highlight.__active:not(.__hidden)").withText("om"),
    );
  },
);
