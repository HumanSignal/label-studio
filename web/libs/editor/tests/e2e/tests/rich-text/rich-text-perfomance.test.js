...
function generateAnnotationParams(sybolsNum, regionsNum, seed = 42) {
  ...
  while (
    (currentWordLength = Math.min(
      randomInt(MIN_WORD_SIZE, MAX_WORD_SIZE),
      sybolsNum - length,
    )) > 0
  ) {
    ...
    annotations: [
      {
        ...
        result: Array.from({ length: regionsNum }, (v, idx) => {
          ...
          const endOffset =
            startOffset + Math.min(randomInt(1, 50), text.length - startOffset);
...
Scenario(
  "Rich text initialization in hightload conditions",
