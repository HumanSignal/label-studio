...
Scenario(
  "Creating, removing and restoring regions",
  async ({ I, LabelStudio, AtOutliner, AtRichText }) => {
    I.amOnPage("/");
    ...
    within({ frame: ".lsf-richtext__iframe" }, () => {
      I.say(
        "We can see all the regions inside the rich text and their styles in head.",
      );
...
Scenario(
  ...
  async ({ I, LabelStudio, AtOutliner, AtRichText }) => {
    ...
    const checkThatRegionsDoNotAffectContent = async (
      startOffset,
      endOffset,
    ) => {
