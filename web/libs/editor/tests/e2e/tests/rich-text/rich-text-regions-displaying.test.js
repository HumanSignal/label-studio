...
Scenario(
  ...
  async ({ I, LabelStudio, AtLabels, AtRichText }) => {
    I.amOnPage("/");
    LabelStudio.init({
      config: `<View>
...

    {
      ...
      const selectedColor = await I.grabCssPropertyFrom(
        elementLocator,
        "background-color",
      );
    ...
    }
    {
      ...
      I.say(`The color of it is ${selectedColor}`);
      assert(
        selectedColor.includes("(0, 255, 0"),
        "Oh no! Green is not green!",
      );
    }
    {
      I.pressKey("u");
      ...
      I.pressKey("u");
      const unselectedColor = await I.grabCssPropertyFrom(
        elementLocator,
        "background-color",
      );
      ...
      assert(
        unselectedColor.includes("(0, 0, 255"),
        "Oh no! Blue is not blue!",
      );
    }
    {
      I.pressKey("u");
      ...
      I.say(`The color of it is ${unselectedColor}`);
      assert(
        unselectedColor.includes("(255, 255, 0"),
        "Oh no! Yellow is not yellow!",
      );
    }
  },
);
...
Scenario(
  "Displaying selected and highlighted regions",
  async ({ I, LabelStudio, AtOutliner, AtDetails }) => {
...

    {
      ...
      const originalBorder = await I.grabCssPropertyFrom(
        ".htx-highlight",
        "border",
      );

      {
        ...
        const hoveredBorder = await I.grabCssPropertyFrom(
          ".htx-highlight",
          "border",
        );
      ...
      }
      {
        ...
        I.moveCursorTo("#logo");
        const unhoveredBorder = await I.grabCssPropertyFrom(
          ".htx-highlight",
          "border",
        );
      ...
      {
        I.say(
          'The other way to get hovered state id using the "Create relation" tool...',
        );
...

    {
      ...
      const originalBackground = await I.grabCssPropertyFrom(
        ".htx-highlight",
        "background",
      );

      AtOutliner.clickRegion(1);
      const selectedBackground = await I.grabCssPropertyFrom(
        ".htx-highlight",
        "background",
      );
...
Scenario(
  "Displaying label in the region",
  async ({ I, LabelStudio, AtOutliner, AtSettings }) => {
    async function checkLabelVisibility(
      locator,
      content,
      shouldBeVisible = true,
    ) {
      const text = await I.grabCssPropertyFromPseudo(
        locator,
        "content",
        "after",
      );
      const display = await I.grabCssPropertyFromPseudo(
        locator,
        "display",
        "after",
      );
      ...
      assert.strictEqual(
        text,
        content,
        `Label name should be "${content}" but "${text}" was seen.`,
      );
      if (shouldBeVisible) {
