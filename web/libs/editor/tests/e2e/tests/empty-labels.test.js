...
examples.forEach((example) => {
  ...
  Scenario(
    `Nonexistent label -> ${title}`,
    async ({ I, LabelStudio, AtSidebar, AtImageView, AtAudioView }) => {
      ...
      if (regionsCount) {
        ...
        for (let i = result.length; i--; ) {
          Asserts.deepEqualWithTolerance(
            Helpers.omitBy(result[i], isLabels),
            restored[i],
            1,
          );
        }
      ...
      if (regionsCount) {
        ...
        for (let i = result.length; i--; ) {
          Asserts.deepEqualWithTolerance(
            Helpers.omitBy(
              result[i],
              (val, key) => key === "from_name" || isLabels(val, key),
            ),
            Helpers.omitBy(
              restored[i],
              (val, key) => key === "from_name" || isLabels(val, key),
  ...
  Scenario(
    `Nonexistent from_name -> ${title}`,
    async ({ I, LabelStudio, AtTopbar, AtSidebar }) => {
      const params = { annotations: [{ id: "test", result }], data };
      const configTree = Utils.parseXml(config);
...
[SINGLE_TYPE, MULTIPLE_TYPE].forEach((type) => {
  Scenario(
    `Making labels empty -> choice="${type}"`,
    async ({ I, LabelStudio, AtSidebar, AtAudioView, AtLabels }) => {
      async function expectSelectedLabels(expectedNum) {
        const selectedLabelsNum = await I.grabNumberOfVisibleElements(
          AtLabels.locateSelected(),
        );

        assert.strictEqual(selectedLabelsNum, expectedNum);
      }
      async function clickLabelWithLengthExpection(
        labelNumber,
        expectedLength,
        expectSelectedNum,
      ) {
      ...
      async function clickLabelWithSelectedExpection(
        labelNumber,
        expectSelectedNum,
      ) {
        AtLabels.clickLabel(`${labelNumber}`);
      ...
      Asserts.deepEqualWithTolerance(
        Helpers.omitBy(result[0], isLabels),
        Helpers.omitBy(restored[0], isLabels),
        1,
      );
...
Scenario(
  "Consistency of empty labels",
  async ({ I, LabelStudio, AtSidebar, AtImageView, AtLabels }) => {
