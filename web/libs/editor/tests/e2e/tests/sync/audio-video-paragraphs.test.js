  ...
  (flags) => {
    FFlagScenario(
      ...
      async ({ I, LabelStudio, AtAudioView, AtVideoView }) => {
...

        {
          ...
          const [{ currentTime: startingVideoTime }] =
            await AtVideoView.getCurrentVideo();
        ...
        I.wait(1);
        {
          ...
          const [{ paused: paragraphAudioPaused }, { paused: audioPaused }] =
            await AtAudioView.getCurrentAudio();
        ...
        {
          I.say(
            "Audio, Video and Paragraph Audio are played to the same time and are now paused",
          );

          const [
            {
              currentTime: currentParagraphAudioTime,
              paused: paragraphAudioPaused,
            },
            { currentTime: currentAudioTime, paused: audioPaused },
          ] = await AtAudioView.getCurrentAudio();
          const [{ currentTime: currentVideoTime, paused: videoPaused }] =
            await AtVideoView.getCurrentVideo();
    ...
    FFlagScenario(
      ...
      async ({ I, LabelStudio, AtAudioView, AtVideoView }) => {
        ...
        I.wait(1);
        {
          ...
          const [, { currentTime: currentAudioTime, paused: audioPaused }] =
            await AtAudioView.getCurrentAudio();
          const [{ currentTime: currentVideoTime, paused: videoPaused }] =
            await AtVideoView.getCurrentVideo();
    ...
    FFlagScenario(
      ...
      async ({ I, LabelStudio, AtAudioView, AtVideoView }) => {
        ...
        {
          const [, { currentTime: startingAudioTime }] =
            await AtAudioView.getCurrentAudio();
          const [{ currentTime: startingVideoTime }] =
            await AtVideoView.getCurrentVideo();
        ...
        {
          I.say(
            "Seek to beginning by clicking on the first point in the audio timeline",
          );
          const [, { currentTime: startingAudioTime }] =
            await AtAudioView.getCurrentAudio();
          const [{ currentTime: startingVideoTime }] =
        ...
        {
          I.say(
            "Seek to end by clicking on the last point in the audio timeline",
          );
          const [, { currentTime: startingAudioTime }] =
            await AtAudioView.getCurrentAudio();
          const [{ currentTime: startingVideoTime }] =
    ...
    FFlagScenario(
      ...
      async ({ I, LabelStudio, AtAudioView, AtVideoView }) => {
        ...
        {
          const [
            { playbackRate: paragraphAudioPlaybackRate },
        ...
        {
          I.say(
            "Changing playback speed to 1.5x for audio, video and paragraph audio during playback",
          );
          const [
            { playbackRate: paragraphAudioPlaybackRate },
            { playbackRate: audioPlaybackRate },
        ...
        {
          I.say(
            "Changing playback speed to 1x for audio, video and paragraph audio during playback",
          );
          const [
            { playbackRate: paragraphAudioPlaybackRate },
            { playbackRate: audioPlaybackRate },
          ...
          assert.equal(
            audioPlaybackRate,
            paragraphAudioPlaybackRate,
            "Audio and Paragraphs speed rate is equal",
          );
          assert.equal(
            videoPlaybackRate,
            audioPlaybackRate,
            "Video and Audio speed rate is equal",
          );
        ...
        AtAudioView.clickPauseButton();
        {
          ...
          const [_, { currentTime: currentAudioTime }] =
            await AtAudioView.getCurrentAudio();
          const [{ currentTime: currentVideoTime }] =
            await AtVideoView.getCurrentVideo();
