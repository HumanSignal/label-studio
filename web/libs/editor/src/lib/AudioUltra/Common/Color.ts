...
export const rgba = (color: string | RgbaColorArray): RgbaColorArray => {
  ...
  if (hexMatch) {
    ...
    return new RgbaColorArray([...arr.slice(0, 3).map((x) => parseInt(x, 16)), parseInt(arr[3] || "ff", 16) / 255] as [
      number,
      number,
      number,
      number,
  ...
  if (rgbaMatch) {
    ...
    return new RgbaColorArray([...arr.slice(0, 3).map((x) => parseInt(x, 10)), parseFloat(arr[3] || "1")] as [
      number,
      number,
      number,
      number,
