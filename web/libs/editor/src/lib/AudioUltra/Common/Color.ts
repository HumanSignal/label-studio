...
export const colorToInt = (x: string) =>
  Number.parseInt(x.replace(/_/g, ""), 36);
...
export const reducedHexRegex = new RegExp(
  `^#${repeat("([a-f0-9])", 3)}([a-f0-9])?$`,
  "i",
);
export const hexRegex = new RegExp(
...
export const rgba = (color: string | RgbaColorArray): RgbaColorArray => {
  if (typeof color !== "string" && !((color as any) instanceof RgbaColorArray))
    throw new Error(
      `Color must be a string or an instanceof RgbaColorArray. Received ${JSON.stringify(
        color,
      )}`,
    );
