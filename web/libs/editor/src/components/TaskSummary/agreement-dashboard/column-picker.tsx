...
export const ColumnPicker = ({
  ...
  const categoricalsOnlyActive = useMemo(() => {
    if (categoricalDimensionIds.length === 0 || visibleColumnIds.length !== categoricalDimensionIds.length)
      return false;
  ...
  return (
    <div className="flex items-center gap-tight w-full">
      ...
      <div className="ml-auto flex items-center gap-tight shrink-0">
        {/* Conflict legend — text-only; chip color (negative-content) communicates the "red" state */}
        <span className="inline-flex items-center text-label-small text-neutral-content-subtle leading-none whitespace-nowrap">
          {hasExistingGt
            ? "Items marked red show conflicts with ground truth"
            : "Items marked red show conflicts with most common answer"}
        </span>
