...
export interface DimensionMeta {
  ...
  /** The dimension's overall weight in project's control_weights (0..1).
   *  Defaults to 1.0 when no weight is configured. A weight of 0 means the
   *  dimension is excluded from the overall agreement score, but the column
   *  is still shown for context. */
  overall_weight?: number;
  /** Whether the labeling-config tag is marked `required="true"`. Defaults to
   *  false (Label Studio's default), in which case the tag is optional and
   *  reviewers can skip it when building Ground Truth. */
  is_required?: boolean;
}
...
export interface DimensionInfo {
  ...
  /** Overall weight in the project's control_weights (0..1). 0 means the
   *  dimension is excluded from the overall agreement score. */
  overallWeight: number;
  /** Whether the labeling-config tag is `required="true"`. When false the tag
   *  is optional and may be skipped when building Ground Truth. */
  isRequired: boolean;
}
...
export interface MajorityVoteResult {
  /** Scalar for single-select dimensions, array for multi-select. */
  value: string | number | boolean | (string | number | boolean)[] | null;
  count: number;
  total: number;
  isTie: boolean;
  tiedValues: (string | number | boolean | (string | number | boolean)[] | null)[];
}

/** Backend-computed most common annotator answer for one dimension. */
export interface MostCommonAnswerDimension {
  name: string;
  value: unknown;
  metric_type: string;
  count: number;
...
export interface GroundTruthInferenceResponse {
  ...
  /** Most common answer from annotator annotations, independent of saved GT state. */
  most_common_answer?: {
    dimensions: Record<string, MostCommonAnswerDimension>;
    annotator_ids: number[];
  };
}
