...
export const Phrases = observer(
  ({ item, playingId, activeRef, setIsInViewport }) => {
    ...
    const setSeekAnimation = useCallback(
      (isSeeking) => {
        ...
        const duration =
          item._value[playingId]?.duration ||
          item._value[playingId]?.end - item._value[playingId]?.start;
        ...
        if (startValue > 0 && startValue < 100)
          animateElement(
            activeRef.current?.querySelector(".reading-line"),
            startValue,
            seekDuration,
            seek.playing,
          );
    ...
    const val = item._value.map((v, idx) => {
      ...
      const withFormattedTime = (item) => {
        ...
        const endTime = formatTime(
          !item._value[idx]?.end
            ? item._value[idx]?.start + item._value[idx]?.duration
            : item._value[idx]?.end,
        );
      ...
      if (getRoot(item).settings.showLineNumbers)
      ...
      return (
        <div
          ...
          className={`${classNames.join(" ")} ${
            isFF(FF_LSDV_E_278) && styles.newUI
          }`}
          style={style?.phrase}
        >
          ...
          {isFF(FF_LSDV_E_278) ? (
            <span className={styles.wrapperText}>
              {isActive && (
                <span
                  ref={readingLineRef}
                  className={`${styles.readingLine} reading-line`}
                  data-skip-node="true"
                />
              )}
