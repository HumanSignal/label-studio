...
const HtxTextAreaResult = observer(
  ({ item, control, firstResultInputRef, onFocus, collapsed }) => {
    const value = item.mainValue;
    const editable =
      !item.isReadOnly() && item.from_name.editable && !item.area.isReadOnly();
...
const HtxTextAreaRegionView = observer(
  ({ item, area, collapsed, setCollapsed, outliner, color }) => {
    const rows = Number.parseInt(item.rows);
    const isTextArea = rows > 1;
    const isActive = item.perRegionArea === area;
    const shouldFocus =
      area.isCompleted &&
      area.perRegionFocusTarget === item &&
      area.perRegionFocusRequest;
    ...
    if (isTextArea) {
      ...
      props.onKeyDown = (e) => {
        if (
          ((e.key === "Enter" && !e.shiftKey) || e.key === "Escape") &&
    ...
    const showAddButton =
      !item.annotation.isReadOnly() && (item.showsubmitbutton ?? rows !== 1);
    const itemStyle = {};
    ...
    const showSubmit =
      (!result ||
        ...
        (item.maxsubmissions &&
          result.mainValue.length < Number.parseInt(item.maxsubmissions))) &&
      !area.isReadOnly();
...
Registry.addPerRegionView(
  "textarea",
  PER_REGION_MODES.REGION_LIST,
  HtxTextAreaRegionView,
);
