...
function sanitizeHtml(html = []) {
  ...
  // Every HTML, SVG and MathML event handler content attribute is reserved by
  // spec to begin with "on" (see the "on*" IDL attribute family in the HTML
  // Living Standard and the SVG event attributes list); no non-handler
  // attribute uses that prefix. A fixed list of handler names has to be kept
  // in sync by hand and misses whatever a reviewer didn't think of - this
  // list omitted the SMIL handlers (onbegin, onend, onrepeat), the animation
  // and transition handlers (onanimationstart/end/iteration,
  // ontransitionstart/end/run/cancel), onfocusin/onfocusout, and the
  // pointer/touch handlers (onpointer*, ontouch*), all of which fire without
  // any user interaction. Matching the reserved prefix removes the whole
  // class instead of one enumerated entry at a time, and does not need to be
  // revisited when a future browser or spec adds another handler.
  const isEventHandlerAttribute = (attr) => /^on/i.test(attr);
    ...
    transformTags: {
      ...
      "*": (tagName, attribs) => {
        Object.keys(attribs).forEach((attr) => {
          // Strip any event handler content attribute, not just enumerated names
          if (isEventHandlerAttribute(attr)) {
            delete attribs[attr];
          }
        });
        return {
