...
const mockSpans = () => {
  const spans = [
    { style: {}, className: "", setAttribute: vi.fn(), scrollIntoView: vi.fn(), scrollIntoViewIfNeeded: vi.fn() },
    { style: {}, className: "", setAttribute: vi.fn() },
  ];
  spans.forEach((s) => {
    s.onmouseover = null;
    s.onmouseout = null;
...
vi.mock("../../utils/html", () => ({
  highlightRange: vi.fn((_self, _cssClass, _cssStyle) => mockSpans()),
}));
...
vi.mock("../../utils", () => ({
  ...
  default: {
    Colors: {
      convertToRGBA: vi.fn((color, alpha) => (color ? `rgba(0,0,0,${alpha})` : null)),
      rgbaChangeAlpha: vi.fn((_color, alpha) => `rgba(0,0,0,${alpha})`),
    },
    HTML: {
      labelWithCSS: vi.fn(() => "htx-label-test"),
    },
  },
}));
...
describe("SpanTextMixin", () => {
  ...
  describe("applyCSSClass", () => {
    ...
    it("sets className when parent.showlabels and settings.showLabels", () => {
      const { model } = getTestTree();
      const span = { style: {}, className: "", setAttribute: vi.fn() };
  ...
  describe("addEventsToSpans", () => {
    ...
    it("onmouseover when hidden does nothing", () => {
      ...
      expect(() => spans[0].onmouseover({ stopPropagation: vi.fn() })).not.toThrow();
    });
    ...
    it("onmouseover when not linking sets pointer cursor", () => {
      ...
      const spans = mockSpans();
      model.addEventsToSpans(spans);
      spans[0].onmouseover({ stopPropagation: vi.fn() });
