...
const RenderElement = memo(({ element, context }: { element: Element; context: RenderContext }) => {
  ...
  switch (tagName) {
    ...
    case 'header': {
      const size = parseInt(getAttr(element, 'size', '4'));
    ...
    case 'video': {
      ...
      return (
        <div 
          ...
          className={`lsf-static-preview__${tagName}`}
        >
          <div style={{ 
            background: '#f0f0f0', 
            padding: '20px', 
    ...
    case 'textarea': {
      ...
      const rows = parseInt(getAttr(element, 'rows', '3'));
      return (
        <textarea 
    ...
    case 'rating': {
      const maxRating = parseInt(getAttr(element, 'maxrating', '5'));
      return (
        <div style={{ ...style }} className="lsf-rating">
          {Array.from({ length: maxRating }, (_, i) => (
            <span key={i} style={{ fontSize: '20px', color: '#ccc' }}>★</span>
          ))}
        </div>
      );
    }
...
export const StaticConfigPreview = memo(({ config, data = {}, className }: StaticConfigPreviewProps) => {
  ...
  return (
    <div className={`lsf-static-preview ${className || ''}`}>
      {renderedContent}
    </div>
  );
});
