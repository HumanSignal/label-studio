...
function deburr(string: string): string {
  return string.replace(reLatin, (letter) => deburredLetters[letter] || letter).replace(reComboMark, "");
}
...
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional control character ranges from lodash's word-splitting regex
 ...
 */
export const camelCase: (str: string) => string = createCompounder((result, word, index) => {
