	...
	render() {
		...
		if (window?.APP_SETTINGS?.flags?.automax_enabled) {
			const fullscreenSizeReduction =
				mods.fullscreen &&
				(this.props.fullscreenSizeReduction ||
					this.props.fullscreenSizeReduction === 0)
						...
						>
							{!bare && (
								<ModalHeader>
									...
									{this.props.header && (
										<Elem name="header-content">{this.props.header}</Elem>
									)}
							...
							{this.props.footer && (
								<ModalFooter bare={this.props.bareFooter}>
									{this.footer}
								</ModalFooter>
							)}
						</Elem>
	...
	transition(type: "visible" | "appear" | "disappear", onFinish: () => void) {
		...
		return aroundTransition(this.modalRef.current, {
			...
			beforeTransition: async () =>
				new Promise((resolve) => {
					this.setState(
						{ transition: `before-${type as "appear" | "disappear"}` },
						() => {
							resolve();
						},
					);
