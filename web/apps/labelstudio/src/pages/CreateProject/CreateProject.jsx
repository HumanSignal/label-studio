    ...
    >
      ...
      {isFF(FF_LSDV_E_297) && (
        <div className="w-full flex flex-col gap-2">
          ...
          <Select placeholder={t("projects:selectAnOption")} disabled options={[]} triggerClassName="!flex-1" />
...
export const CreateProject = ({ onClose }) => {
  ...
  const steps = {
    name: <span className={tabClass.mod({ disabled: !!error }).toClassName()}>{t("projects:projectName")}</span>,
    import: (
      <span className={tabClass.mod({ disabled: uploadDisabled }).toClassName()}>{t("projects:dataImport")}</span>
    ),
    config: t("projects:labelingSetup"),
  };
        ...
        <Modal.Header>
          ...
          <Space>
            <Button
              ...
              onClick={onDelete}
              waiting={waiting}
              aria-label={t("projects:cancelProjectCreation")}
            >
              {t("projects:cancel")}
            </Button>
            <Button
