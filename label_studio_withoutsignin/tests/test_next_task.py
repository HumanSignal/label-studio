...
def test_next_task_with_active_learning(
    ...
    for task, prediction, annotation in zip(tasks, predictions, annotations):
        ...
        if annotation is not None:
            completed_by = (
                any_client.annotator if num_annotators == 1 else annotator2_client.annotator
            )
...
def test_label_race_with_overlap(configured_project, business_client):
    ...
    r = business_client.patch(
        ...
        data=json.dumps(
            {"maximum_annotations": 2, "overlap_cohort_percentage": 50, "show_overlap_first": True}
        ),
        content_type="application/json",
    )
    assert r.status_code == 200
...
def test_fetch_final_taken_task(business_client):
    ...
    ann1.post(
        f"/api/tasks/{task_id}/annotations/", data={"task": task_id, "result": annotation_result}
    )
...
def test_with_bad_annotation_result(business_client):
    ...
    for i in range(num_annotators):
        anns.append(
            make_annotator({"email": f"ann{i}@testwithbadannotationresult.com"}, project, True)
        )
    ...
    with mock.patch("tasks.models.update_project_summary_annotations_and_is_labeled"):
        for i in range(10):
            task = make_task(
                {"data": {"image": f"https://data.s3.amazonaws.com/image/{i}.jpg"}}, project
            )
            for i in range(num_annotators):
                make_annotation(
                    {
    ...
    uncompleted_task = make_task(
        {"data": {"image": f"https://data.s3.amazonaws.com/image/uncompleted.jpg"}}, project
    )
...
def test_overlap_first(business_client, setup_before_upload, show_overlap_first):
    ...
    if setup_before_upload:
        r = c.patch(
            f"/api/projects/{project.id}/",
            data=json.dumps(
                {"maximum_annotations": 2, "overlap_cohort_percentage": overlap_cohort_percentage}
            ),
            content_type="application/json",
        )
        assert r.status_code == 200
