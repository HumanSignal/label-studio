...
def gcs_client_mock():
    ...
    class DummyGCSBlob:
        ...
        def download_as_string(self):
            ...
            if self.is_json:
                return json.dumps(
                    {"str_field": data, "int_field": 123, "dict_field": {"one": "wow", "two": 456}}
                )
            return data
...
def azure_client_mock():
    ...
    with mock.patch.object(
        models.BlobServiceClient, "from_connection_string", return_value=DummyAzureClient()
    ):
...
def upload_data(client, project, tasks):
    ...
    return client.post(
        f"/api/projects/{project.id}/tasks/bulk", data=data, content_type="application/json"
    )
...
def login(client, email, password):
    if User.objects.filter(email=email).exists():
        ...
        r = client.post(
            f"/user/signup/", data={"email": email, "password": password, "title": "Whatever"}
        )
        assert r.status_code == 302, r.status_code
